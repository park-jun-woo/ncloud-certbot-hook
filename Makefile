...
@if [ ! -f "$(CONFIG_FILE)" ]; then read -p "Enter your NCP Access Key: " ACCESS_KEY; read -p "Enter your NCP Secret Key: " SECRET_KEY; echo '{\n  "access_key": "'SECRET_KEY'",\n  "root_ca_path": "/etc/ssl/certs/ISRG_Root_X1.pem",\n  "sleep_time": 30\n}' | sudo tee $(CONFIG_FILE); sudo chown root:root $(CONFIG_FILE); sudo chmod 600 $(CONFIG_FILE); fi
